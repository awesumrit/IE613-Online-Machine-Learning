...
class Learner:
	...
	def Exp3(self, ita, t):
		...
		for round_number in range(total_prediction_rounds):
			...
			Xt = Loss[0] if (round_number)<=T/4 else Loss[1]
			Xtp = Loss[0] if (round_number)<=T/4 else Loss[1]
			Xtix = Loss[0] if (round_number)<=T/4 else Loss[1]

			x = np.array([1-Xt/probs[i] if (i == 0 if (round_number)<=T/4 else 1)  else 1 for i in range(K) ])
			xp = np.array([(1-Xtp+beta)/probsp[i] if (i == 0 if (round_number)<=T/4 else 1) else beta/probsp[i] for i in range(K) ])
			xix = np.array([(1-Xtix)/(gammaix+probsix[i]) if (i == 0 if (round_number)<=T/4 else 1) else 0 for i in  range(K)])
