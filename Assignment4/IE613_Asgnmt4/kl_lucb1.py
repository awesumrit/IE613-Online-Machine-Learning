...
def kl_L_UCB(mu,num_arms):
	...
	while B > epsilon:
		Jt = mu_hat.argsort()[-m:][::-1]
		values[upper_t] += np.random.binomial(n=1, p= mu[upper_t], size=1)
		values[lower_t] += np.random.binomial(n=1, p= mu[lower_t], size=1)
		counts[upper_t] += 1
		counts[lower_t] += 1 
		mu_hat = values/counts
